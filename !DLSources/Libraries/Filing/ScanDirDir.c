...
static os_error *Filing__ScanDirByDirRec(const char *dirname, filing_fulldirentry *dirdata, scandir_funcs *funcs)
{
 ...
 do {
   ...
   if (actptr->objtype==2) {
     ...
     if (er=Filing__ScanDirByDirRec(newname,actptr,funcs),er!=NULL) {
       return er;
     }
   }
 } while(TRUE);
                              ...
                              Filing_ScanDir_EndDir *enddirproc)
{
 ...
 if (dirname[strlen(dirname)-1]!='$') {
   ...
   if (er=Filing_SingleFullDirEntry(dirname,&dirdata,sizeof(dirdata)),er!=NULL)
     return er;
   return Filing__ScanDirByDirRec(dirname,&dirdata,&funcs);
 }
 else {
   /* it could be possible to 'forge' the filing_fulldirentry for the root directory... */
   return Filing__ScanDirByDirRec(dirname,NULL,&funcs);
 }
}
